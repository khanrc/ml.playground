...
class STNs(object):
    def __init__(self, name="stn", lr=0.001, locnets_type='fc', use_residual_M=False):
        self.name = '{}-{}'.format(name, locnets_type)
        # self.locnets_type = locnets_type
        # self.lr = lr
    ...
    def _build_nets(self, lr, locnets_type, use_residual_M):
        with tf.variable_scope(self.name):
            ...
            x_img = tf.reshape(self.X, [-1, 40, 40, 1])
            ...
            with tf.variable_scope("locnets-" + locnets_type):
                ...
                if locnets_type == 'conv':
                    ...
                    n_filters = 32
                    for i in range(3):
                        loc_net = conv(loc_net, n_filters, kernel_size=[3,3])
                        loc_net = max_pooling(loc_net, pool_size=[2,2], strides=2)
                        n_filters *= 2
                    # 32, 64, 128
                    # 20x20, 10x10, 5x5
                    ...
                    # cnn => dense
                    loc_net = dense(self.X, 256, activation=tf.nn.relu)
                else:
                ...
                if use_residual_M == False:
                    ...
                    M = dense(loc_net, 6, kernel_initializer=tf.zeros_initializer, bias_initializer=init)
                else:
            ...
            with tf.variable_scope("cnn"):
                ...
                for i in range(3):
                    net = conv(net, n_filters, kernel_size=[3,3])
                    net = max_pooling(net, pool_size=[2,2], strides=2)
                    n_filters *= 2
